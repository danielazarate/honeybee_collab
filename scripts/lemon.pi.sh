...
docker run \
    --mount type=bind,source=/media/data/dazarate,target=/mnt/bee.stuff \
    --mount type=bind,source=/media/data/dazarate/scripts.out,target=/mnt/bee.out \
    --memory=120g \
    angsd3 \
...
REF="/mnt/bee.stuff/European_genome_scaffolds/Amel_4.5_scaffolds.fa"
...
/opt/angsd/angsd/angsd -out "$DIR_OUT/$POP" \
-r Group1:1000000-1100000 \
-anc "$REF" \
-fold 0 \
-underFlowProtect 1 \
-bam "$BAM_LIST" \
